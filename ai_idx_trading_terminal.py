...
def detect_bandar(df):
    if df["volume"] > 1.5 * df["vol_avg"] and df["close"] > df["open"]:
        return "🐋 Akumulasi"
    elif df["volume"] > 1.5 * df["vol_avg"] and df["close"] < df["open"]:
        return "⚠️ Distribusi"
    return "-"


def get_top_50(df):
    return df.sort_values(by="score", ascending=False).head(50)


# === UI TAMBAHAN ===
st.subheader("🔍 Cari Saham Manual")
search_stock = st.text_input("Masukin kode saham (contoh: ADRO, BRIS, TLKM)")

if "df_scan" in st.session_state:
    df_scan = st.session_state["df_scan"]

    if search_stock:
        code = search_stock.upper() + ".JK"
        if code in df_scan["stock"].values:
            result = df_scan[df_scan["stock"] == code]
...
with col1:
    if st.button("🏆 Top 50 Elite"):
        mode = "ELITE"

with col2:
    if st.button("🚨 Alert Breakout"):
        mode = "ALERT"

with col3:
    if st.button("🐋 Bandar Only"):
        mode = "BANDAR"
...
if 'df_scan' in locals():
    if mode == "ELITE":
        df_display = df_scan.sort_values(by="score", ascending=False).head(50)

    elif mode == "ALERT":
        df_display = df_scan[df_scan["score"] > 70]

    elif mode == "BANDAR":
        df_display = df_scan[df_scan["bandar"].str.contains("AKUMULASI")]
...
with col1:
    if st.button("🔊 Toggle Sound Alert"):
        st.session_state.alert_sound = not st.session_state.alert_sound

with col2:
    if st.button("💬 Toggle Popup Alert"):
        st.session_state.alert_popup = not st.session_state.alert_popup


# === APPLY ALERT LOGIC ===
if 'df_scan' in locals():
    alert_df = df_scan[(df_scan["score"] > 70) & (df_scan["bandar"].str.contains("AKUMULASI"))]

    if not alert_df.empty:
        top_alert = alert_df.iloc[0]

        if st.session_state.alert_popup:
            st.warning(f"🚨 ALERT: {top_alert['stock']} berpotensi breakout!")

        if st.session_state.alert_sound:
            st.audio("https://www.soundjay.com/buttons/sounds/beep-07.mp3")


# === 📰 NEWS SYSTEM (NEXT LEVEL) ===
import requests

st.subheader("📰 Berita Saham Syariah")

if st.button("🔄 Refresh Berita"):
    try:
        url = "https://newsapi.org/v2/everything?q=indonesia%20stock&language=en&pageSize=10&apiKey=YOUR_API_KEY"
        ...
        def simple_sentiment(text):
            positive_words = ["gain", "growth", "profit", "surge", "rise"]
            negative_words = ["loss", "drop", "fall", "decline", "risk"]
        ...
        for art in articles:
            ...
            # mapping sederhana ke saham
            related = []
            for s in df_scan["stock"].tolist()[:200]:
                if s.replace(".JK","") in title.upper():
                    related.append(s)

            st.markdown(f"**{title}**")
            st.markdown(f"Sentimen: {sentiment}")
            if related:
                st.markdown(f"Terkait saham: {', '.join(related[:5])}")
            st.markdown("---")

    except Exception as e:
        st.error("Gagal ambil berita. Pastikan API key valid.")
...
# Apply clean stock name before display
if 'df_display' in locals():
    df_display = df_display.copy()
    df_display["stock"] = df_display["stock"].apply(clean_stock)

    st.subheader("📊 Hasil Screener (Clean View)")
    st.dataframe(highlight_top(df_display), use_container_width=True)

# Fix search display
if 'result' in locals():
    result = result.copy()
    result["stock"] = result["stock"].apply(clean_stock)
    st.subheader("🔍 Hasil Pencarian")
    st.dataframe(highlight_top(result), use_container_width=True)
